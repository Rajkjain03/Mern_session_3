...
const Signin = ({ onToggle }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch('http://localhost:5001/api/users/signin', { //
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password }),
      });
      const data = await res.json();
      if (res.ok) dispatch(setCredentials(data)); //
  ...
  return (
    <div style={{ maxWidth: '400px', margin: 'auto', padding: '20px' }}>
      ...
      <p style={{ textAlign: 'center', marginTop: '20px' }}>
        ...
        <button type="button" onClick={onToggle} style={{ background: 'none', border: 'none', color: 'blue', cursor: 'pointer' }}>Sign Up</button>
      </p>
    </div>
  );
};
